...
Tool calls are **dual-routed**: client-defined tools (passed in `req.tools`) become
...
|---------|--------|---------|
| **Tool execution loop** | 🔴 opencode-specific | When the agent makes a `tool_call`, Rosetta converts it to a `FunctionCall` output item. There is no loop to execute the tool and send results back to the agent. This means tool-dependent workflows (e.g., web search, file operations) won't complete. |
...
| **Tool call execution loop** | When the agent makes a `tool_call`, Rosetta converts it to a `FunctionCall` output item. There is no loop to execute the tool and send results back to the agent. | 🔜 Future |
